...
/**
 * Name of the active Hermes agent profile (host.state.profile) — for messaging
 * only. The gateway's config.set already writes the active profile's config, so
 * this is informational and never used to gate the write.
 */
function activeProfileName() {
  try {
    const value = host?.state?.profile?.get?.();
...
/**
 * Apply a voice persona the Hermes-native way, to the ACTIVE AGENT PROFILE:
 ...
 *
 * config.set is intentionally called WITHOUT session_id. The Hermes gateway is
 * bound to the active profile, so a session-less config.set writes that
 * profile's config (~/.hermes/profiles/<name>/config.yaml) — the persona then
 * applies across every chat in that profile and, because profiles are isolated,
 * never touches other agent profiles. Passing a session_id would instead pin the
 * persona to a single chat so it would not stick across the profile (the
 * original bug). This mirrors persistHermesTtsVoice(), which also omits
 * session_id as the reliable profile-wide path.
 */
async function applyHermesPersona({ key, prompt, voiceName }) {
  ...
  return { profileName, modes, voiceName };
}
        ...
        try {
          ...
          host.notify({
            ...
            message: result.profileName
              ? `"${voiceName}" persona active for profile "${result.profileName}" (${mode}).`
              : `"${voiceName}" persona active for the active profile (${mode}).`,
          });
        } catch (err) {
          host.notify({
            kind: 'info',
